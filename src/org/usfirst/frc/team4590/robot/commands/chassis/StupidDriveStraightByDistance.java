 ...
 */
public class StupidDriveStraightByDistance extends Command implements PIDOutput, PIDSource {
	...
	public StupidDriveStraightByDistance(double distance, double angle, boolean reset){
	...
	public StupidDriveStraightByDistance(double distance, double angle){
		this(distance, angle, false);
	}
	public StupidDriveStraightByDistance(double distance) {
	...
	protected boolean isFinished() {
		return Math.abs(Chassis.getInstance().getDistance() * 319.185) >= Math.abs(m_distance); 
	}
	...
	public void pidWrite(double output) {
		...
		Chassis.getInstance().arcadeDrive(-Math.signum(m_distance) * 0.6, P2 * (m_angle - Chassis.getInstance().getAngle()));
	}
}
